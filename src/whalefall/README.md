...
│   ├── skill                        按需加载 skills/<path>/SKILL.md
...
│   ├── sleep / config
│
├── skills/                          Skill 文档库（按 domain 嵌套 SKILL.md）
│   ├── general/weather/SKILL.md
│   └── demo/nested/{alpha,beta}/SKILL.md
│
├── mcp/                             MCP 协议层 + 本地演示 server
...
用户输入
 ...
 │               压缩触发且 ctx.recently_read/invoked_skills/todo 非空时
 │               _insert_after_last_system 插入 0~3 条 <system-reminder>
...
[F] MCP_INSTRUCTIONS         「[MCP Server 使用说明]」+ 每个 server 的 instructions 聚合
                             来源：MCPClient.list_instructions()
                             （MCP spec：server 在 initialize 响应里声明 instructions）
                             按 agent.allowed_mcp_servers 过滤
...
### 3.2 整个对话里会出现的所有 `role=system` 消息（共 7 类）

| # | 名字 | 位置 | 何时出现 | 代码位置 |
| --- | --- | --- | --- | --- |
| S1 | idx 0 主 system（上面 A-G 的拼接） | 进 while 前一次装一次 | 必然 | `loop.py::_build_system_prompt` |
| S2 | `<system-reminder>父 Agent 上下文</...>` | 进 while 前 | 仅子 agent 被父唤起 | `loop.py` L213 |
| S3 | `<system-reminder>session_start hook 上下文</...>` | 进 while 前 | 注册了 hook 且返回 `additional_context` | `loop.py` L252 |
| S4 | `<system-reminder>未完成任务</...>` | 进 while 前 | `todo_store.active_tasks()` 非空 | `loop.py` L282 |
| S5 | `<system-reminder>压缩后文件上下文恢复</...>` | while 循环内 | 压缩触发 **且** `ctx.recently_read` 非空 | `loop.py` L320 |
| S6 | `<system-reminder>压缩后 Skill 上下文恢复</...>` | while 循环内 | 压缩触发 **且** `ctx.invoked_skills` 非空 | `loop.py` L325 |
| S7 | `<system-reminder>压缩后任务列表恢复</...>` | while 循环内 | 压缩触发 **且** `store.active_tasks()` 非空 | `loop.py` L330 |

- S2~S7 都统一用 `parts.py::wrap_system_reminder(body, title=...)` 包成
...
| 工具使用指引 / MCP instructions / skill 目录清单 | ❌ | ✅（换 agent / 改权限时） |
...
- Skill 目录清单 —— 直接写进 `skill` 工具的 `function.description` 里
  （`SkillTool._dynamic_tool_description` 按 agent `allowed_skill_paths` 动态生成）
- 每个 MCP 工具的 function schema（名字前缀 `<server>__<tool>`）
...
| `allowed_skill_paths` | `None / [] / ["dir/"]` | `None` | `None` | 同上；以 `/` 结尾是目录前缀，不以 `/` 结尾是精确匹配 |
...
allowed_skill_paths: [general/, finance/stock/]        # 可选，skill 前缀白名单
...
whalefall 的工具分**三大类**；它们通过不同通道被 LLM 看到，但对 LLM 来说都是同一协议的
"function call"。
...
- `to_openai_schema(agent_config)` 返回 function schema；支持按 agent 动态生成
  （`SkillTool` 在这里把可见 skill 目录塞进 `description`）
...
| `skill` | R | 按名加载 `skills/<path>/SKILL.md` |
...
### 4.2 Skill（"按需加载的文档"，不是独立协议）

Skill 是一种**文档形态的工具扩展**——`skills/<domain>/<name>/SKILL.md` 放步骤、
SOP、领域小抄。LLM 通过**内建 `skill` 工具**按名加载全文：

```
skills/
├── general/weather/SKILL.md           # name=weather
└── finance/stock/factor/SKILL.md      # name=finance/stock/factor（相对路径即名）
```

关键点：

- **LLM 怎么知道能调哪些 skill**？`skill` 工具的 `function.description` 里附带当前 agent
  可见的 skill 目录（`- weather: ...`）——通过 `tools=[...]` 参数而不是 system prompt 传达。
- `SKILL.md` frontmatter 的 `description` 会作为目录项摘要；正文在工具调用时完整返回。
- 调用后记录到 `ctx.invoked_skills`，autocompact 触发时会被 S6 自动恢复。
- Agent 级过滤（`allowed_skill_paths`）**两端校验**：
  list 端（LLM 看不到被禁的 skill）+ execute 端（即使 LLM 硬猜路径也 load 不了）。
...
- 每个 MCP server 可在 `initialize` 响应里声明 `instructions` 字段，
  `MCPClient._server_instructions` 会存下来，`collect_mcp_instructions` 聚合进 [F]。
...
### 4.4 一张图对照三类工具的"能见度"通道

```
LLM 看到的东西:
├── messages[0]  (system prompt)
│   ├── [E] TOOL_REFERENCES          ← 内建工具的 prompt() 文本拼接
│   └── [F] MCP_INSTRUCTIONS         ← MCP server instructions 聚合
│
└── tools=[...]  (function schema 列表)
    ├── 每个 BuiltinTool.to_openai_schema()
    ├── skill 工具的 description 里列出 "可用技能：- weather: ..."
    └── 每个 MCP 工具（名字前缀 <server>__<tool>）
```
...
- `allow_subagent: false` 的 agent 禁止再嵌套；子 Agent 也可配自己的
  `allowed_mcp_servers` / `allowed_skill_paths` 做沙箱隔离
...
Step 3   DEFAULT_ALLOW_TOOLS → ALLOW
         {read, glob, grep, agent, skill, web_search,
...
from whalefall.agent.roles import (
    ...
    wrap_system_reminder, collect_mcp_instructions,
...
| 内建工具 | **全开**（16+） | `--no-builtin` 全关；或 `build_default_registry(include_web_search=False, ...)` 粒度控制 |
...
2. **事实与规则分离**：规则（BASE / GUARDRAILS / TONE_STYLE）静态常量；事实（`<env>` / 工具指引 / MCP instructions）动态渲染。
...
6. **所有 agent / skill / MCP 插件都在 `src/whalefall/` 内**——不做 CC 那种项目级/用户级多源加载，**零文件嗅探**（框架永不读取 cwd 下的 `*.md`）。
